...
//Inventaris
    ...
    public function laboratoriumSupport()
    {
        $token = session('api_token');
        $response = Http::withToken($token)->get(env('API_URL') . '/laboratorium-support');
        ...
        return view('Admin.LaboratoriumSupport', compact('laboratoriumSupports'));
    }

    public function laboratoriumSupportDetail($id)
    {
        $token = session('api_token');
        $response = Http::withToken($token)->get(env('API_URL') . '/laboratorium-support/' . $id);

        if ($response->successful()) {
            $laboratoriumSupport = $response->json()['data'];
        } else {
            return redirect()->back()->with('error', 'Gagal mengambil data detail');
        }

        return view('Admin.LaboratoriumSupportDetail', compact('laboratoriumSupport'));
    }

    public function laboratoriumSupportTambah()
    {
        return view('Admin.LaboratoriumSupportTambah');
    }

    public function laboratoriumSupportTambahPost(Request $request)
    {
        $token = session('api_token');
        $data = [
            'name' => $request->name,
        ...
        $response = Http::withToken($token)->post(env('API_URL') . '/laboratorium-support', $data);

        if ($response->successful()) {
            return redirect()->route('laboratorium-support.admin')->with('message', 'Berhasil menambahkan data')->with('alert-type', 'success');
        } else {
            $errorBody = $response->body();
            return redirect()->back()->with('error', 'Gagal: ' . $errorBody)->with('alert-type', 'error');
        }
    ...
    public function laboratoriumSupportEdit($id)
    {
        $token = session('api_token');
        $response = Http::withToken($token)->get(env('API_URL') . '/laboratorium-support/' . $id);
        ...
        return view('Admin.LaboratoriumSupportEdit', compact('laboratoriumSupport'));
    }

    public function laboratoriumSupportUpdate(Request $request, $id)
    {
        $token = session('api_token');
        $data = [
            'name' => $request->name,
        ...
        $response = Http::withToken($token)->put(env('API_URL') . '/laboratorium-support/' . $id, $data);

        if ($response->successful()) {
            return redirect()->route('laboratorium-support.admin')->with('message', 'Berhasil memperbarui data')->with('alert-type', 'success');
        } else {
            $errorBody = $response->body();
            return redirect()->back()->with('error', 'Gagal: ' . $errorBody)->with('alert-type', 'error');
        }
    ...
    public function laboratoriumSupportHapus($id)
    {
        $token = session('api_token');
        $response = Http::withToken($token)->delete(env('API_URL') . '/laboratorium-support/' . $id);

        if ($response->successful()) {
            return redirect()->route('laboratorium-support.admin')->with('message', 'Berhasil menghapus data')->with('alert-type', 'success');
        } else {
            $errorBody = $response->body();
            return redirect()->back()->with('error', 'Gagal: ' . $errorBody)->with('alert-type', 'error');
        }
