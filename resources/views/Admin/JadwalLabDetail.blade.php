        ...
        <div class="flex justify-between items-center">
                ...
                <table class="min-w-full bg-white rounded ">
                    <thead class="bg-[rgba(98,143,142,0.2)] text-black">
                        <tr>
                            ...
                            <th class="py-3 px-4 text-left border-b-2 border-gray-300 text-lg">Mata Kuliah</th>
                            ...
                            <th class="py-3 px-4 text-left border-b-2 border-gray-300 text-lg"></th>
                        </tr>
                    </thead>
                    <tbody class="text-black bg-[rgba(98,143,142,0.2)]  ">
                        <!-- Repeat this block for each lab schedule -->
                         @foreach($room['schedules'] as $jadwal)
                        <tr class="border-t">
                            <td class="py-3 px-4 text-lg">{{\Carbon\Carbon::parse($jadwal['start_time'])->setTimezone(config('app.timezone'))->format('H:i')}}
                            </td>
                            <td class="py-3 px-4 text-lg">{{\Carbon\Carbon::parse($jadwal['end_time'])->setTimezone(config('app.timezone'))->format('H:i')}}</td>
                            <td class="py-3 px-4 text-lg">{{$jadwal['subject']['subject_name']}}</td>
                            <td class="py-3 px-4 text-lg">{{$jadwal['dosen']}}</td>
                            <td class="py-3 px-4 text-lg">{{$jadwal['information']}}</td>
                            <td class="py-3 px-4 text-lg">
                            <div class="flex flex-row gap-4 w-full md:w-auto">
                    <a href="{{ Route('jadwallabedit.admin',$jadwal['id']) }}"
                        class=" shadow-lg hover:scale-105 flex flex-row gap-3 items-center bg-[#499DBC] px-4 py-2  justify-center w-full md:w-auto rounded-md">
                        <button
                            class="  font-medium text-md lg:text-lg text-white flex items-center">
                            Edit
                        </button>
                    ...
                    <!-- <a href="#"
                        class=" hover:scale-105 shadow-md flex flex-row gap-2 items-center bg-[#D46857] px-4 py-2 rounded-md justify-center w-full md:w-auto">
            ...
            <div class="flex flex-col-reverse md:flex-row md:items-center md:justify-between gap-4 py-4">
                <div
                    ...
                    <a href="{{ Route('jadwallab.admin') }}?date={{$selectedDate}}"
                        class="  font-medium text-md lg:text-lg text-white flex items-center">
                        Kembali
                    </a>
                </div>
